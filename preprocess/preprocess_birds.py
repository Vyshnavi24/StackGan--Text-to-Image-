...
IMSIZE = 256
LOAD_SIZE = IMSIZE
BIRD_DIR = '/Users/shrinidhi/study/StudyMaterial/sem2/machine_learning/project/StackGAN/Data/birds'
...
def load_bbox(data_dir):
    bbox_path = data_dir + '/CUB_200_2011/bounding_boxes.txt'
    print(bbox_path)
    df_bounding_boxes = pd.read_csv(bbox_path,
                                    delim_whitespace=True,
                                    header=None).astype(int)
    #
    filepath = data_dir + '/CUB_200_2011/images.txt'
...
def save_data_list(inpath, outpath, filenames, filename_bbox):
    ...
    for key in filenames:
        ...
        img = get_image(f_name, (lr_size, lr_size), is_crop=True, bbox=bbox)
        #img = img.astype('uint8')
        # print(img)
        #hr_images.append(np.array(img))
        #lr_img = transform.resize(img, [lr_size, lr_size])
        ...
        if cnt % 100 == 0:
            print('Load %d......' % cnt)
    #
    print('images', len(lr_images), lr_images[0].shape)
    #
